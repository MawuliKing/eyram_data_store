<?php
// admin/orders.php (Fully Updated: Manual Completion Feature + RESTORED CARD COLORS)

include_once '_partials/header.php';
if (session_status() === PHP_SESSION_NONE) session_start();

// --- Block to display success/error messages from our new feature ---
$message = null;
...
<!-- ✅ THIS IS THE RESTORED STYLE BLOCK THAT FIXES THE CARD COLORS -->
<style>
...
<div class="container-fluid">
    ...
    <?php if ($message): ?>
        <div class="alert alert-<?= htmlspecialchars($message['type']) ?> text-center" role="alert">
    ...
    <h1 class="h2 mb-3 text-center">Manual Order Completion</h1>
    <form method="POST" action="manual_complete.php" class="row g-3 mb-5 align-items-end p-3 bg-light rounded border" onsubmit="return confirm('Are you sure you want to force-complete all matching PROCESSING orders? This action cannot be undone.');">
        <div class="col-md-3">
            <label for="comp_start_date" class="form-label">Start Date (Optional)</label>
            <input type="date" class="form-control" id="comp_start_date" name="start_date">
        </div>
        <div class="col-md-3">
            <label for="comp_end_date" class="form-label">End Date (Optional)</label>
            <input type="date" class="form-control" id="comp_end_date" name="end_date">
        </div>
        <div class="col-md-3">
            <label for="category_to_complete" class="form-label">Select Service to Complete</label>
            <select class="form-select" id="category_to_complete" name="category_to_complete" required>
                <option value="" disabled selected>-- Choose a Service --</option>
                <option value="Data Bundle - MTN">MTN Data</option>
                <option value="Data Bundle">AirtelTigo Data</option>
                <option value="Data Bundle-Telecel">Telecel Data</option>
                <option value="Mashup">MTN Mashup</option>
                <option value="Exam Results">BECE/WASSCE Results</option>
                <option value="Exam Vouchers">School Placement</option>
            </select>
        </div>
        <div class="col-md-3">
            <button type="submit" class="btn btn-warning w-100">
                <i class="fas fa-check-double"></i> Force Complete Orders
            </button>
        </div>
    </form>
...
<script>
// The JavaScript remains the same as the previous 'cleaned up' version.
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.category-card').forEach(card => {
        card.addEventListener('click', function() {
            loadOrders({ group_key: this.dataset.groupKey }, `Orders for ${this.dataset.groupName}`);
        });
    });

    document.getElementById('showAllBtn').addEventListener('click', () => loadOrders({}, 'All Recent Orders'));
